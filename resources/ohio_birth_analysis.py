...
def ethnicity_pie(race_df):
    ...
    fig.suptitle('Percentage of Low Birth Weights per Ethnicity',
                 fontsize=16
                 )
...
def county_breakdown_plot(age_df, county_df):
    ...
    # Plot creation.
    fig = ff.create_choropleth(fips=county_fips,
                               values=percent_low,
                               scope=['OH'],
                               binning_endpoints=low_bins,
                               colorscale=_Themes.low_colorscale,
                               legend_title='% of county births',
                               asp=3,
                               show_hover=True,
                               width=800,
                               height=400,
...
def race_breakdown_plot_stacked(race_df):
    ...
    race_ser = race_df.groupby(by=['year', 'race',
                                   'weight_indicator'
                                   ]
                               ).birth_count.sum()
    race_sort_df = race_ser.unstack()
    race_sort_df.loc[2006]
